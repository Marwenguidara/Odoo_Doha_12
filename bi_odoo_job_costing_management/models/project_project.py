...
class Project(models.Model):
    ...
    @api.multi
    @api.depends('drawing_ids.total_drawing')
    def _compute_total_drawing(self):
        for line in self:
            total_drawing = 0.0
            for rec in line.drawing_ids:
                total_drawing += rec.total_drawing
            line.update({
    ...
    @api.multi
    @api.depends('drawing_ids.total_prod')
    def _compute_total_prod(self):
        for line in self:
            total_prod = 0.0
            for rec in line.drawing_ids:
                total_prod += rec.total_prod
            line.update({
                'total_prod': total_prod,
            })

    @api.multi
    @api.depends('drawing_ids.total_deli')
    def _compute_total_deli(self):
        for line in self:
            total_deli = 0.0
            for rec in line.drawing_ids:
                total_deli += rec.total_deli
            line.update({
                'total_deli': total_deli,
            })

    @api.multi
    @api.depends('drawing_ids.total_erec')
    def _compute_total_erec(self):
        for line in self:
            total_erec = 0.0
            for rec in line.drawing_ids:
                total_erec += rec.total_erec
            line.update({
    ...
    def print_quotation(self):
        return self.env.ref('drawing.report_bill_quotation_1').report_action(self)
    ...
    def button_view_issue(self):
        ...
        if total_issue:
            project_project = self.env['ir.actions.act_window'].for_xml_id('bi_odoo_job_costing_management',
                                                                           'project_issue_categ_act0')
